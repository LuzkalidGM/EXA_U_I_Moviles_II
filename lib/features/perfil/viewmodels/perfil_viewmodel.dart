...
class PerfilViewModel extends ChangeNotifier {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (data != null) {
          ...
          notifyListeners();
          unawaited(_runBackgroundTask(_userSyncService.syncCurrentUser));
  ...
  Future<void> _runBackgroundTask(Future<void> Function() task) async {
    try {
      await task().timeout(const Duration(seconds: 15));
    } catch (e, st) {
      developer.log(
        'VM sincronización en segundo plano: $e',
        error: e,
        stackTrace: st,
      );
    }
  ...
  void startHeartbeat({Duration interval = const Duration(seconds: 45)}) {
    ...
    final user = supabase.auth.currentUser;
    if (user == null) return;
    unawaited(_runBackgroundTask(_userSyncService.updateCurrentUserLastSeen));
  ...
  Future<LoginResult> login({
    ...
    try {
      ...
      await _loadProfile();
      // El listener de autenticación actualiza notificaciones y presencia.
